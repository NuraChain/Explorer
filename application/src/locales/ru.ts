...
export const ru: Dictionary = {
    ...
    // --- Verified source ----------------------------------------------------------------------
    'contract.verified.full': 'Исходный код проверен',
    'contract.verified.partial': 'Исходный код проверен - частичное совпадение',
    'contract.verified.detail': '{name}, скомпилирован solc {compiler}',
    'contract.verified.partial.hint': 'Развёрнутый код совпадает; различается только хвост метаданных, а для этого достаточно перенесённого комментария или другого пути к файлу. Инструкции доказаны, комментарии вокруг них - нет.',
    'contract.verified.viaImplementation': 'Опубликованный исходный код принадлежит реализации, к которой обращается этот адрес.',
    'contract.source': 'Исходный код',
    'contract.source.optimized': 'оптимизатор включён, {runs} прогонов',
    'contract.source.unoptimized': 'оптимизатор выключен',
    'contract.source.copyAbi': 'Копировать ABI',

    // --- Publishing source --------------------------------------------------------------------
    'verify.cta': 'Опубликовать исходный код',
    'verify.title': 'Проверка {short}',
    'verify.heading': 'Публикация и проверка исходного кода',
    'verify.intro': 'Здесь ничто не принимается на веру. Обозреватель компилирует присланное и принимает его только тогда, когда результат совпадает с байт-кодом, уже развёрнутым по этому адресу, - поэтому не нужно ни учётной записи, ни чьего-либо разрешения.',
    'verify.offline': 'Список компиляторов получить не удалось, поэтому доступны только сборки, уже лежащие на этом сервере. Попросите администратора добавить нужную версию.',
    'verify.kind': 'Формат отправки',
    'verify.kind.single': 'Один файл',
    'verify.kind.json': 'Standard JSON',
    'verify.compiler': 'Сборка компилятора',
    'verify.compiler.hint': 'Именно сборка, а не только выпуск - вывод solc меняется между патч-версиями. Точкой отмечены сборки, уже имеющиеся на сервере.',
    'verify.evmVersion': 'Версия EVM',
    'verify.evmVersion.default': 'По умолчанию у компилятора',
    'verify.evmVersion.hint': 'Оставьте значение по умолчанию, если контракт не собирался с явно заданной версией. Неверная меняет получаемые опкоды.',
    'verify.name': 'Имя контракта',
    'verify.name.hint': 'Необязательно. Если оставить пустым, будут проверены все контракты в исходниках.',
    'verify.fileName': 'Имя файла',
    'verify.fileName.hint': 'Путь, по которому файл компилировался. Он попадает в хеш метаданных, поэтому неверный превращает полное совпадение в частичное.',
    'verify.optimizer': 'Оптимизатор',
    'verify.optimizer.enabled': 'Включён',
    'verify.optimizer.hint': 'Эти настройки должны совпадать с теми, с которыми контракт был развёрнут, иначе байт-код не совпадёт.',
    'verify.runs': 'Прогоны оптимизатора',
    'verify.license': 'Лицензия',
    'verify.source': 'Исходный код Solidity',
    'verify.source.placeholder': 'Вставьте контракт целиком, вместе с импортами.',
    'verify.json': 'Ввод Standard JSON',
    'verify.json.placeholder': 'Вставьте standard-json, который выдала ваша сборка.',
    'verify.json.hint': 'Его настройки применяются ровно так, как написаны, - вместе с remappings, адресами библиотек и viaIR. Расширяется только выбор вывода.',
    'verify.submit': 'Проверить и опубликовать',
    'verify.submit.hint': 'Компиляция занимает несколько секунд, и одновременно выполняется одна отправка.',
    'verify.failed.hint': 'Несовпадение почти всегда сводится к сборке компилятора, настройке оптимизатора или числу прогонов. Сверьте каждое с тем, с чем контракт был развёрнут.',
    'verify.done.detail': 'Исходный код {name} опубликован для этого контракта, и его функции теперь названы и доступны для вызова.',
    'verify.done.open': 'Открыть контракт',
