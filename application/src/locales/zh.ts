...
export const zh: Dictionary = {
    ...
    // --- Verified source ----------------------------------------------------------------------
    'contract.verified.full': '源码已验证',
    'contract.verified.partial': '源码已验证 - 部分匹配',
    'contract.verified.detail': '{name}，由 solc {compiler} 编译',
    'contract.verified.partial.hint': '已部署的代码一致，仅元数据尾部不同；一条注释挪动位置或文件路径不同就足以造成这种差异。指令已被证明，围绕它们的注释没有。',
    'contract.verified.viaImplementation': '公布的源码属于该地址转发到的实现合约。',
    'contract.source': '源代码',
    'contract.source.optimized': '优化器开启，{runs} 次运行',
    'contract.source.unoptimized': '优化器关闭',
    'contract.source.copyAbi': '复制 ABI',

    // --- Publishing source --------------------------------------------------------------------
    'verify.cta': '公布源码',
    'verify.title': '验证 {short}',
    'verify.heading': '公布并验证源码',
    'verify.intro': '这里不依赖任何信任。浏览器会编译你提交的源码，只有当结果就是该地址上已部署的字节码时才接受 - 因此无需注册账户，也不必等待任何人批准。',
    'verify.offline': '无法获取编译器列表，因此只能使用本服务器上已有的构建。请让运维添加你需要的版本。',
    'verify.kind': '提交格式',
    'verify.kind.single': '单个文件',
    'verify.kind.json': 'Standard JSON',
    'verify.compiler': '编译器构建',
    'verify.compiler.hint': '要精确到构建，而不只是发行版本 - solc 的输出在补丁版本之间也会变化。圆点标出本服务器上已有的构建。',
    'verify.evmVersion': 'EVM 版本',
    'verify.evmVersion.default': '编译器默认值',
    'verify.evmVersion.hint': '除非合约编译时明确指定过，否则保持默认。指定错误会改变生成的操作码。',
    'verify.name': '合约名称',
    'verify.name.hint': '可选。留空则会逐一尝试源码中的每个合约。',
    'verify.fileName': '文件名',
    'verify.fileName.hint': '文件编译时所用的路径。它会被哈希进元数据，因此填错会把完全匹配变成部分匹配。',
    'verify.optimizer': '优化器',
    'verify.optimizer.enabled': '启用',
    'verify.optimizer.hint': '这些必须与合约部署时的设置一致，否则字节码不会一致。',
    'verify.runs': '优化器运行次数',
    'verify.license': '许可证',
    'verify.source': 'Solidity 源码',
    'verify.source.placeholder': '粘贴完整合约，包括 import。',
    'verify.json': 'Standard JSON 输入',
    'verify.json.placeholder': '粘贴构建产生的 solc standard-json 输入。',
    'verify.json.hint': '其设置会原样使用 - 包括 remappings、库地址与 viaIR。只有输出选择会被放宽。',
    'verify.submit': '验证并公布',
    'verify.submit.hint': '编译需要几秒钟，且同一时间只运行一个提交。',
    'verify.failed.hint': '不匹配几乎总是出在编译器构建、优化器设置或运行次数上。请逐项对照合约部署时的配置。',
    'verify.done.detail': '{name} 的源码已公布在该合约上，其函数现在有名字，也可以调用。',
    'verify.done.open': '打开合约',
